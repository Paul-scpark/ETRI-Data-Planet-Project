...
def get_data(URL):
    ...
    section_lst = soups.select('#tab-layer-openapi > .api-meta-table-mobile > table > tr')
    if len(section_lst) == 0:
        section_lst = soups.select('#data-search-view > #file-meta-table-pc > table > tr')
    ...
    if len(section_dic.keys()) > 0:
        ...
        with open('data_go_kr_data.csv', 'a', newline='') as csvfile:
            ...
            if csvfile.tell() == 0: csv_writer.writeheader()
