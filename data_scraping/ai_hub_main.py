...
def get_data_info(li):
    ...
    try: # 페이지마다 조금 형식이 다른 부분이 있어서 try, except으로 예외처리 해주기
        ...
        table_col = [th.text for th in table_selector.select_one('thead > tr').select('th')]
        table_row = [span.text for span in table_selector.select_one('tbody > tr').select('td > span')]
...
def get_data_structure(li):
    ...
    for tr in li.select('div > table > tbody > tr'):
        ...
        detail_lst += [li_td.text.replace('\t', '').replace('\n', '').replace('  ', '') for li_td in tr.select('td')]
...
if __name__ == '__main__':
    ...
    for page in tqdm(range(1, 21)):
        ...
        soups_json = get_json_using_bs4('https://www.aihub.or.kr/aihubdata/data/aJaxDataList.do', data)
    ...
    for data_idx, i in enumerate(tqdm(data_lst[123+70:])):
        with webdriver.Chrome(service=Service(ChromeDriverManager().install())) as driver:
            ...
            data_title = soups.select_one('#content > div.tabDiv1 > div.dataHead > div > h3').text
            for idx, li in enumerate(soups.select('#content > div.tabDiv1 > div.contentWrap > ul > li')):
                # AI HUB 홈페이지에서 '데이터 개요' 부분
                if idx == 0: soup_sub1 = get_data_info(li)
                # AI HUB 홈페이지에서 '메타데이터 구조표' 부분
                elif idx == 1: soup_sub2 = get_data_structure(li)
            ...
            with open('ai_hub_data.csv', 'a', newline='') as csvfile:
                ...
                if data_idx == 0: csv_writer.writeheader()
