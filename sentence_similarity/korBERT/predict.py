...
def predict(config, text):
    ...
    encoder = ['경제금융', '교육과학', '교통물류', '기타', '문화관광', '보건의료', '사회복지', '식품건강',
       '재난안전', '제조소비', '주택토지', '통일외교안보', '행정법률', '환경기상']
    ...
    predict_data = [[text, 0]]
    predict_dataset = korBERTDataset(predict_data, 0, 1, tok, vocab, config.max_len, True, False)
